...
import { SECTION_IDS, TEACHER_STORY } from '@/lib/content/landing-copy';
...
const heatmapLevelLabels = ['Madal', 'Alla keskmise', 'Keskmine', 'Üle keskmise', 'Kõrge'];
...
export function TeacherSection() {
  ...
  const handleHeatmapKeyDown = useCallback(
          ...
          <div className='relative mx-auto max-w-4xl bg-elevated rounded-xl p-3 sm:p-6 border border-border overflow-hidden'>
            <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4'>
              ...
              <span className='text-text-secondary text-sm font-mono'>
                {students} õpilast · {skills} oskust
              </span>
            </div>
                    ...
                    {Array.from({ length: students }).map((_, row) => (
                      ...
                      >
                        {Array.from({ length: skills }).map((_, col) => {
                          ...
                          return (
                            <td
                              // biome-ignore lint/suspicious/noArrayIndexKey: generated heatmap cells — col index is the only identity, list never reorders
                              key={`${row}-${col}`}
                              className='p-0'
                            >
                              <button
            ...
            <div className='flex items-center justify-center gap-4 mt-3 pt-3'>
              ...
              <div className='flex gap-1'>
                {[1, 2, 3, 4, 5].map((level) => (
                  <div
                    key={level}
                    className={`w-6 h-3 rounded-sm heatmap-cell-${level}`}
                    style={{ opacity: 0.6 }}
                  />
                ))}
              </div>
            ...
            <div
              ...
              style={{
                background:
                  'radial-gradient(circle at var(--heatmap-x, 50%) var(--heatmap-y, 50%), rgba(30, 90, 138, 0.1), transparent 30%)',
              }}
            />
          </div>
        </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto'>
            {teacherSignals.map((signal, index) => (
              ...
              >
                ...
                <div className='text-sm font-medium text-text-primary mb-2'>{signal.label}</div>
        ...
        <div className='max-w-4xl mx-auto'>
          <div className='bg-green-50 rounded-xl p-6 border border-green-200 dark:bg-surface dark:border-success-border'>
            <div className='flex items-start justify-between mb-4'>
              <h3 className='text-lg font-semibold text-green-900 dark:text-success-strong'>
                Õpetaja sekkumine
              </h3>
            ...
            <div className='space-y-3 mb-4'>
              <div>
                <div className='text-xs font-medium text-green-800 uppercase tracking-wider mb-1 dark:text-success-strong'>
                  Soovitus
                </div>
                <div className='text-sm text-green-900 dark:text-success-strong'>
              ...
              <div>
                ...
                <div className='text-sm text-green-700 dark:text-success-strong'>
                  {PRODUCT_FIXTURE.teacherAction.evidence}
                </div>
              </div>
            </div>

            <div className='flex gap-3 flex-wrap pt-4 border-t border-green-200 dark:border-success-border'>
              <div className='text-xs font-medium text-green-800 uppercase tracking-wider dark:text-success-strong'>
                Õpetaja valikud:
              </div>

...
export function StatusFilterSection({
  ...
  return (
    <div>
      <div role="group" aria-label="Filtreeri staatuse järgi" className="flex flex-wrap gap-2 mb-4">
        <span className="sr-only" aria-live="polite">
          {filter === 'Kõik'
            ? 'Näidatakse kõiki'
            : `Näidatakse ainult staatust: ${filter}`}
        </span>
        {FILTERS.map((f) => (
          <button
            key={f}
          ...
          >
            {f}
            <span className={`text-xs tabular-nums ${filter === f ? 'opacity-80' : 'text-text-secondary'}`}>
              {countFor(f)}
            </span>
          </button>
        ))}
      ...
      {planned.length > 0 && (
        <details className="group mt-4 rounded-xl border border-border bg-elevated">
